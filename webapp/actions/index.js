...
export function getJoinURL(channelId, meetingid, creatorid) {
  return async (dispatch, getState) => {
    ...
    try {
      ...
      //alert("url from getjoin: "+url.url);
      return {
        data: {
          joinurl: url
        }
...
export function startMeeting(channelId, description = '', topic = '', meetingId = 0) {
  return async (dispatch, getState) => {
    try {
      const resp = await Client.startMeeting(getState().entities.users.currentUserId, channelId, topic, description);

      //check response from server
      //alert("from server: "+JSON.stringify(resp));

      //get join URL:
      let url;
      var ismod = "TRUE";
      try{
        url = await Client.getJoinURL(getState().entities.users.currentUserId, resp.meeting_id, ismod);
      } catch (error) {
        const post = {
          id: 'bbbPlugin' + Date.now(),
          create_at: Date.now(),
      ...
      //alert("got join URL: "+ url.url);
      window.open(url.url);

    } catch (error) {
      var message_text = 'Did not successfully start a meeting';
